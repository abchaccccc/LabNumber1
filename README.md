...
2. **Сравнение чисел:**  
   ...
   работы с `B`. Если `A` больше, а `X`>0, то программа увеличивает `count` на 1. Если `X`=0, то программа ничего не делает. Затем программа проводит аналогичные действия для `B` и `C`, затем программа проверяет, является
