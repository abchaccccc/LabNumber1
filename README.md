...
Всего надо рассмотреть `1+4*2=9` случаев.
...
2. **Сравнение чисел:**  
   ...
   работы с `B`. Если `A` больше, программа увеличивает `count` на 1. Затем программа проводит аналогичные действия для `B` и `C`, затем программа проверяет, является
